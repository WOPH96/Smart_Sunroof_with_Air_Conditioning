...
### 2.1 상태 모니터링  
클러스터의 화면 구성은 다음과 같습니다.
![image](https://github.com/user-attachments/assets/876605da-e98a-4db4-a95e-eda1e69d61d4)
...
차량 상태 표시창을 자세히 보면 아래와 같습니다.
![image](https://github.com/user-attachments/assets/520696af-998f-4bd9-87db-6548eb3a3b77)

현재 동작 여부 : 창문 액추에이터가 동작중일 때는 해당 박스가 초록색으로 변화하고, 동작중이 아닐때는 회색 박스로 되돌아 옵니다.  
현재 액추에이터 상태 : 창문의 현재 개폐상태를 나타냅니다. 0%가 최대로 열린 상태이고, 100%가 최대로 닫힌 상태 입니다.  
전원 On/Off 여부 : 액추에이터의 전원 상태를 나타냅니다. 시동 버튼을 통해 사용자가 시동을 키면 전원이 On이 되며, 시동이 켜져있을 때, 시동 버튼을 누르면 시동이 꺼지며 모든 액추에이터의 전원이 종료됩니다.  
현재 CAN 연결상태 : CAN 메세지를 수신받았을 때, 정상으로 표시되며, CAN 메세지를 수신받지 못했으면 비정상으로 화면에 표시됩니다.  
스마트 On/Off 여부 : 기본적으로 시동을 키면 스마트제어가 모두 On 되며 스마트 제어버튼을 통해 모든 액추에이터의 스마트 제어 여부를 On 또는 Off 로 변경 할 수 있습니다. 다만, 사용자가 액추에이터를 직접 제어한 경우 스마트 기능이 잠시 해제되며, 일정 시간 후에 다시 ON이 됩니다. 
